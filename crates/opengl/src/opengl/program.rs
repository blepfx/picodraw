use super::GlFramebufferBinding;
use glow::{HasContext, TEXTURE_2D, TEXTURE_BUFFER, TEXTURE0, TRIANGLES, UNIFORM_BUFFER};
use std::{array::from_fn, cell::RefCell};

pub struct GlProgram<T: HasContext> {
    program: T::Program,
    uniforms: RefCell<Vec<Option<T::UniformLocation>>>,
}
...
impl<'a, T: HasContext> GlProgramBinding<'a, T> {
    pub fn set_uniform_binding(&self, gl: &T, name: &str, binding_index: u32) {
        unsafe {
            if let Some(location) = gl.get_uniform_location(self.program.program, name) {
                let mut uniforms = self.program.uniforms.borrow_mut();
                if uniforms.len() <= binding_index as usize {
                    uniforms.resize(binding_index as usize + 1, None);
                }

                uniforms[binding_index as usize] = Some(location);
            }
        }
    }
    ...
    pub fn set_uniform_block_binding(&self, gl: &T, name: &str, binding_index: u32) {
        unsafe {
            if let Some(location) = gl.get_uniform_block_index(self.program.program, name) {
                gl.uniform_block_binding(self.program.program, location, binding_index);
            }
        }
    }
    ...
    pub fn set_uniform_i32(&self, binding_index: u32, value: i32) {
        unsafe {
            if let Some(Some(location)) = self.program.uniforms.borrow().get(binding_index as usize) {
    ...
    pub fn set_uniform_block(&self, binding_index: u32, buffer: T::Buffer) {
        unsafe {
            self.gl.bind_buffer_base(UNIFORM_BUFFER, binding_index, Some(buffer));
        }
    }
    ...
    pub fn set_buffer_texture(&self, sampler_index: u32, texture: T::Texture) {
        unsafe {
            self.gl.active_texture(TEXTURE0 + sampler_index);
            self.gl.bind_texture(TEXTURE_BUFFER, Some(texture));
        }
    }
}
