...
use glow::HasContext;
use picodraw_core2::{Bounds, Size};
...
pub struct Dispatcher<'a, T: HasContext> {
    ...
    drawcall_quads: &'a mut Vec<QuadDescriptorStruct>,
...
impl<'a, T: HasContext> Dispatcher<'a, T> {
    ...
    pub fn push_clear(&mut self, bounds: Bounds) {
        self.flush();
    ...
    pub fn push_object(&mut self, shader: &CompilerShader) {
        if self.object_rects.len() == 0 {
            return;
        }

        if self.object_textures.len() != shader.texture_slots.len() {
            panic!("invalid texture count passed");
        }
        ...
        let buffer_fits = (self.drawcall_data.len() + self.object_data.len())
            + (self.drawcall_quads.len() + self.object_rects.len()) * QuadDescriptorStruct::SIZE
